...
void html_filtrarCategoria(LL_Categ ll){
	...
	printf("<tr>");
		printf("<td align=center>");
			printf("<font face=\"Times New Roman\" size=\"+3\" color=\"#ff0000\">FILTRADO POR CATEGORIAS</font>");
		printf("</td>");
	printf("</tr>");
	printf("</table><hr>");
	...
	while(stack != NULL){
		...
		printf("<tr>");
		printf("<td align=center><b>Chave</h5></b>");
		printf("<td align=center><b>Autor</h5></b>");
		printf("<td align=center><b>Titulo</h5></b>");
		printf("</tr>");

		contador = 0; // vai contar quantas vezes aparece cada categoria
		...
		while(aux2 != NULL){
			...
			if(strcmp(stack->elem,aux2->categ->nome)==0){
				printf("<tr>");
					printf("<td>%s</td>",aux2->categ->chave);
					printf("<td>%s</td>",aux2->categ->autor);
					printf("<td>%s</td>",aux2->categ->titulo);
				printf("</tr>");
				contador++;
			}
		...
		printf("<tr><td><b>TOTAL</b></td><td><b>%d obras</b></td></tr>", contador);
		// vai eliminar o primeiro elemento da stack
		stack = popStack(stack);
		//aux2 volta a apontar para o inicio da lista de categorias
		aux2 = ll;
...
void html_filtrarAutor(LL_Categ ll){
	...
	Esp_Stack stack = NULL;
	Categoria *c = NULL;
	int contador = 0; // para contar o numero de livros de cada autor
	...
	printf("<tr>");
		printf("<td align=center>");
			printf("<font face=\"Times New Roman\" size=\"+3\" color=\"#ff0000\">FILTRADO POR AUTOR</font>");
		printf("</td>");
	printf("</tr>");
	printf("</table><hr>");
	...
	while(stack != NULL){
		...
		printf("<tr>");
		printf("<td align=center><b>Categoria</h5></b>");
		printf("<td align=center><b>Chave</h5></b>");
		printf("<td align=center><b>Titulo</h5></b>");
		printf("</tr>");
		
		contador = 0; // vai contar quantas vezes aparece cada autor
		...
		while(aux2 != NULL){
			...
			if(strcmp(stack->elem,aux2->categ->autor)==0){
				printf("<tr>");
					printf("<td>%s</td>",aux2->categ->nome);
					printf("<td>%s</td>",aux2->categ->chave);
					printf("<td>%s</td>",aux2->categ->titulo);
				printf("</tr>");
				contador++;
...
void gerar_html_b(LL_Categ ll_categ){
	...
	html_filtrarCategoria(ll_categ);
	printf("<hr color=\"#ff0000\" size=\"4\" />");
	printf("<br><br>");
	html_filtrarAutor(ll_categ);
